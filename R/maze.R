...
#' @param i To make sure that the randomness of the created black dots is captured and not repeated.
...
#' filePath = getwd()
#' maze(rank,i,saturation,gridThree,filePath)
#'
#' }

maze <- function(rank,i,saturation, grid, html){
  R <- rank
...
#   }
  ...
  #file.path('~/Dropbox/Cambridge/PhD/ICAR/AIG/Maze/Maze example/', "page",i,".html")
  #htmlfile = paste0('~/Dropbox/Cambridge/PhD/ICAR/AIG/Maze/Maze example/loop/', "seed",i,".html")
  cat(paste0("\n<html><head> <link rel='stylesheet' type='text/css' href='seed",i,".css'>"), append=TRUE,file=htmlfile)
  ...
  ?cat
  cat("\n<p align=\"center\" style=\"font-family:lucida sans unicode,lucida grande,sans-serif;font-size:14px;\"><font color=\"white\">The goal is to collect as many gold coins as possible as you plan your route up to the top.</font></p>", append=TRUE, file=htmlfile)
  cat("\n<p align=\"center\" style=\"font-family:lucida sans unicode,lucida grande,sans-serif;font-size:14px;\"><font color=\"white\">To start, click on the first node at the bottom of the maze.</font></p>", append=TRUE, file=htmlfile)
  cat("\n<input id=\"countdown\" name=\"timeLeft\" type=\"hidden\" />", append=TRUE, file=htmlfile)
  cat("\n<div style=\"text-align: center; font-size:25px\"><font color=\"white\">Timer: <span id=\"countdown2\">&nbsp;</span></font></div>", append=TRUE, file=htmlfile)
  cat(paste0("<script src='seed",i,".js'></script>"),append=TRUE, file=htmlfile)
  ...
  length(start.index)
  for(k in 1:length(start.index)){
    start.node.coord[k,]<- coord.node[start.index[k],]
  }
  ...
  for(l in 1:length(end.index)){
    end.node.coord[l,] <- coord.node[end.index[l],]
  }
  ...
  #### Edges of Nodes
  connections = ""
  for (m in 1:nrow(ed.2)){
    if(arrowDirect$direction[m]==2){
      connections <- paste0(connections,"
                            ...
                            <path id=",paste0('"',ed.2[m,1],'_',ed.2[m,2],'"'), " d=","\"M",start.node.coord.1[m,1],',',start.node.coord.1[m,2],',L',end.node.coord.1[m,1],',',end.node.coord.1[m,2],"\" style=\"stroke:black; stroke-width: 3.25px; fill: none ;marker-end: url(#arrow",m,");\" >
                            </path> ")
    }else{
      connections <- paste0(connections,"
                            <path id=",paste0('"',ed.2[m,1],'_',ed.2[m,2],'"'), " d=","\"M",start.node.coord.1[m,1],',',start.node.coord.1[m,2],',L',end.node.coord.1[m,1],',',end.node.coord.1[m,2],"\" style=\"stroke:black; stroke-width: 3.25px; fill: none ;\" >
                            </path> ")
    }
    }
  connections
  ...
  cssfile  = file.path(paste0(html,"/seed",i,".css"))
  ...
  cat(button, append=TRUE, file=cssfile)

  ####

  #### JAVA SCRIPT #####

  #jsfile = paste0('~/Dropbox/Cambridge/PhD/ICAR/AIG/Maze/Maze example/loop/', "seed",i,".js")
  jsfile = file.path(paste0(html, "/seed",i,".js"))
  ...
  arrowDirect
  start.index
  for (n in 1: 1:nrow(ed.2)){
    if (n != 1) {
      edge.list <- paste0(edge.list,",[",start.index[n],",",end.index[n],",", travelledFirstNode[],",", travelledBlackNode[],",",direction[n], "]")
    } else {
      edge.list <- paste0(edge.list,"[",start.index[n],",",end.index[n],",", travelledFirstNode[],",", travelledBlackNode[],",",direction[n],"]")
    }
  }
  ...
  v <- paste0("if(")
  e <- NULL
  for(o in 1:(length(nodePosition)-1)){
    e <- paste0(e,paste0("(nodeclicked.id == ",nodePosition[o],") || "))
  }
  ...
  test1 <- NULL
  for(p in 1:length(nodeLength)){
    test1[[p]] <- end.index.df[which(end.index.df$end.index == nodeLength[p]),]
  }
  lastRow <- do.call("rbind",test1)

  v <- paste0("if(")
  e<- NULL
  for(q in 1:(nrow(lastRow)-1)){
    e <- paste0(e,paste0("(nodeclicked.id == edgeArray[",lastRow$index[q]-1,"][1]) || "))
  }
  e
  ...
  cat(javaScript, append=TRUE, file=jsfile)
  cat(javaScript2, append=TRUE, file=jsfile)
                      }
